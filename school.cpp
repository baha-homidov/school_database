...
void School::output_all(std::ostream& os){    //output all the students int studets_list to ostream
...
void School::save_as_file(std::string filename){   //save data to a text file
...
{       
    auto s=  std::find_if(students_list.begin(),students_list.end(),[id_num] (const Student& s) {return s.id_number==id_num;});
    if(s!=students_list.end()) {
        students_list.erase(s);
        std::cout<<"Student ID: "<<id_num<<" deleted.\n";
    }
    else std::cout<<"No student found with id number: "<<id_num<<".\n";
}
